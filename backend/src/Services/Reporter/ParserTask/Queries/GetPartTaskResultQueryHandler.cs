...
public class GetPartTaskResultQueryHandler : IRequestHandler<GetPartTaskResultQuery, Result<ParserTaskPartialResultDto>>
{
	...
	public GetPartTaskResultQueryHandler(AppDbContext context, ILogger<GetTaskResultsQueryHandler> logger)
	{
		_context = context;
		_logger = logger;
	}

	public async Task<Result<ParserTaskPartialResultDto>> Handle(GetPartTaskResultQuery request, CancellationToken cancellationToken)
	{
		try
		{
